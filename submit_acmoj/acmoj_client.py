...
# -*- coding: utf-8 -*-
"""
ACMOJ API Client Command Line Tool - Git or File Submission v2.3

Usage Examples:
1. Submit Git repo URL (Recommended for this task):
   python3 acmoj_client.py --token ${ACMOJ_TOKEN} submit --problem-id ${ACMOJ_PROBLEM_ID} --git-url <your_repo_https_or_ssh_url>
...
def main():
    ...
    # Submit Git repository or single source file
    submit_parser = subparsers.add_parser("submit", help="Submit Git repo or single file")
    ...
    group.add_argument("--git-url", type=str, help="Git repository URL to submit (language=git)")
    group.add_argument("--code-file", type=str, help="Path to the source code file to submit")
    submit_parser.add_argument("--language", type=str, default="cpp",
                               help="Programming language for file submissions (e.g., cpp, c, python)")
    ...
    if args.command == "submit":
        if getattr(args, 'git-url'.replace('-', '_')):
            result = client.submit_git(args.problem_id, getattr(args, 'git-url'.replace('-', '_')))
        else:
            try:
